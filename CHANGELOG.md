...
### 🔒 Fixed
- `connectNode` now awaits the Nostr client handshake so relay connection failures raise `NodeError` instead of crashing callers with unhandled rejections.

### 🧪 Tests
- Added regression coverage ensuring `connectNode` waits for and surfaces handshake failures.

### 📚 Documentation
- README now highlights the safer connection wrapper and how to catch connection errors.

...
export function awaitShareEcho(groupCredential, shareCredential, options = {}) {
    const { relays = DEFAULT_ECHO_RELAYS, timeout = 30000, eventConfig = { enableLogging: true, logLevel: 'info' } } = options;
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            node = createBifrostNode({ group: groupCredential, share: shareCredential, relays }, customEventConfig);
...
export function startListeningForAllEchoes(groupCredential, shareCredentials, onEchoReceived, options = {}) {
    const { relays = DEFAULT_ECHO_RELAYS, eventConfig = { enableLogging: true, logLevel: 'info' } } = options;
    const nodes = [];
    ...
    shareCredentials.forEach((shareCredential, index) => {
        try {
            const shareDetails = decodeShare(shareCredential);
            const node = createBifrostNode({ group: groupCredential, share: shareCredential, relays }, { ...eventConfig, customLogger });
            nodes.push(node);
 ...
 */
export async function sendEcho(groupCredential, shareCredential, options = {}) {
    const { relays = DEFAULT_ECHO_RELAYS, timeout = 10000, eventConfig = { enableLogging: true, logLevel: 'info' } } = options;
    return new Promise(async (resolve, reject) => {
        let node = null;
        let timeoutId = null;
        let isResolved = false;
        ...
        try {
            ...
            const onEchoResponse = (msg) => {
                if (msg && msg.tag === '/echo/res') {
                    safeResolve(true);
                }
            };
            const onEchoRejection = (reason, msg) => {
